  ...
  return (
    <main className="min-h-screen bg-[#f7f9f6] text-[#171717]">
      <div className="mx-auto max-w-3xl px-5 py-8 sm:px-8 lg:py-12">
        ...
        <h1 className="mt-5 text-3xl font-semibold tracking-tight">Create Project</h1>
        <p className="mt-2 text-gray-500">Give your next idea a name and a direction.</p>
        <form onSubmit={handleSubmit} className="mt-8 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          <fieldset disabled={saving} className="space-y-6 disabled:opacity-70">
            ...
            <div>
              ...
              <input id="name" name="name" required maxLength={120} placeholder="e.g. Build my portfolio" className={inputClass} />
            </div>
            <div>
              <label htmlFor="description" className="text-sm font-medium">Description</label>
              <textarea id="description" name="description" rows={4} maxLength={2000} placeholder="What will you achieve?" className={inputClass} />
            </div>
            <div>
            ...
            <div>
              ...
              <input id="due_date" name="due_date" type="date" max="9999-12-31" className={inputClass} />
          ...
          <div className="mt-8 flex flex-wrap gap-4 border-t border-gray-100 pt-6">
            <button type="submit" disabled={saving} className="rounded-xl bg-[#45634c] px-5 py-3 text-sm font-semibold text-white hover:bg-[#354e3b] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#45634c] disabled:cursor-wait disabled:opacity-60">{saving ? "Creating…" : "Create Project"}</button>
